...
function procuraLetra(palavra){
	alfabeto.addEventListener('click', function(event){
		...
		if(classe == 'q-p' || classe == 'a-ç' || classe == 'z-m'){
			null;
		}else{
				...
				if(letra.toLowerCase() == palavra[i]){
					...
					if(!validaErro){
						if(numeradorImagem <= 6){
							numeradorImagem++;
							imagem.src = 'image/'+numeradorImagem+'-forca.png';
							validaErro= true;
						}
						...
						if(numeradorImagem == 6){
							for(var btn = 0; btn<27;btn++){
								...
								msgFinalizacao.innerHTML = 'Que pena, você perdeu! A palavra era <p style="color: white; padding-top: 5px">"' + palavra + '"</p> Jogue novamente.';
								msgFinalizacao.style.display = 'inline-block';
								msgFinalizacao.style.color = '#780000';
							}
						}
					}
				}
			...
			if(listaAcertos.length > 0){
				...
				if(contadorAcertos == palavra.length){
					imagem.src = 'image/'+numeradorImagem+'-vitoria.png';

					msgFinalizacao.innerHTML = 'Parabens, você venceu! Jogue novamente.';
					msgFinalizacao.style.display = 'inline-block';
					msgFinalizacao.style.color = '#007800';
