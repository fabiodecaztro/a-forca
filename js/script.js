var animais = ['porco', 'pato', 'macaco', 'vaca', 'tamandua', 'coelho', 'tigre', 'girafa', 'flamingo', 'pirarucu'];
var paises = ['brasil', 'alemanha', 'chile', 'china', 'dinamarca', 'espanha', 'gabao', 'zambia', 'listenstaine' ,'botsuana'];
var nomes = ['fabio', 'pedro', 'eduardo', 'tiago', 'fernando', 'roberta', 'jessica', 'nilza', 'juliana', 'carolina'];
var flores = ['rosa', 'orquidea', 'begonia', 'lirio', 'crisantemo', 'hortencia', 'violeta', 'amarilis', 'azaleia', 'portulaca'];
var carros = ['corolla', 'fusca', 'siena', 'toro', 'versa', 'elantra', 'argo', 'civic', 'corcel', 'carajas'];
var add = [];
var listaCategorias = [animais, paises, nomes, flores, carros, add];
var listaNomeCategorias = ['Animais', 'Países', 'Nomes', 'Flores', 'Carros', 'Adicionados'];

let menu = document.querySelector('.menu')
var botoes = document.querySelector('.botoes');
var botaoAnimais = document.querySelector('.animais');
var botaoFlores = document.querySelector('.flores');
var botaoNomes = document.querySelector('.nomes');
var botaoPaises = document.querySelector('.paises');
var botaoCarros = document.querySelector('.carros');
...
botaoFlores.addEventListener('click', function(){
	iniciaJogo(flores, 'Flores');
})
...
botaoCarros.addEventListener('click', function(){
	iniciaJogo(carros, 'Carros');
});
...
botaoAdd.addEventListener('click', function(){
	let inputPalavara = document.querySelector('.input-palavra');
	...
	submitPalavra.addEventListener('click', function(){
		if(inputPalavara.value == ''){
			alert('Adicione uma palavra.')
		}else{			
			add.push(inputPalavara.value.normalize('NFD').replace(/([\u0300-\u036f]|[^0-9a-zA-Z])/g,''));
			alert('Palavra "' + inputPalavara.value + '" adicionada com sucesso.' );
			inputPalavara.value = '';
		}
	});
